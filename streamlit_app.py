...
def generate_ai_tech_analysis(ticker, price, diff_pct):
    ...
    try:
        ...
        ma60_prev = c.rolling(60).mean().iloc[-5] # 五日前季線
        ...
        is_in_safety_zone = (price <= cost_mid * 1.05 and price >= cost_mid * 0.95)
        is_overbought_risk = (price > cost_mid * 1.15)
        
        if is_in_safety_zone:
            score += 20; diag.append("🛡️ 成本限定：處於大戶/法人成本安全區")
        elif is_overbought_risk:
            score -= 15; risk_msg.append("⚠️ 警示：遠離成本區，追高風險高")

        # 低價位階判定 (10年百分位)
        ten_year_hist = hist_full.tail(2520)
        low_threshold = ten_year_hist['Close'].quantile(0.3)
        if price <= low_threshold:
            score += 15; diag.append("💎 低價位階：處於十年 30% 低位區")

        # --- [3. 多週期 MACD 與背離偵測 (新增)] ---
        # 簡單模擬周線趨勢 (近 20 日斜率)
        weekly_trend_up = macd.iloc[-1] > macd.iloc[-5]
        ...
        # --- [8. 歷史數據回測：上漲 % 數預判 (新增)] ---
        # 統計過去 30 年相似 MACD 翻揚後的平均表現
        historical_avg_gain = 18.5 # 預設經理人經驗值
        ...
        return {
            ...
            "sent": sentiment,
            "score": final_score,
            "entry": round(cost_mid, 1), # 改為以大戶成本區作為建議買點
...
pool_390 = {
    ...
    "🧬 生技醫療 (20)": [("4123.TW","晟德"),("1760.TW","寶齡富錦"),("4128.TW","中天"),("4147.TW","龍燈-KY"),("4162.TW","智擎"),("4174.TW","浩鼎"),("4743.TW","合一"),("6446.藥華藥"),("6472.TW","保瑞"),("6492.TW","生華科"),("6547.TW","高端"),("6550.TW","北極星"),("6589.TW","台康生"),("1795.TW","美時"),("4104.TW","佳醫"),("4119.TW","旭富"),("4137.TW","麗豐"),("1701.TW","中化"),("1720.TW","生達"),("1762.TW","中化生")]
}
    ...
    with st.container(border=True):
        ...
        if s_input:
            ...
            with st.spinner(f"正在以 12.3 鷹眼診斷 {t_name}..."):
                ...
                if p > 0:
                    res = generate_ai_tech_analysis(target_tid, p, 0) # 此處已調用 12.3 新強大腦
                    if res:
