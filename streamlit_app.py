...
# ==============================================================================
# 第四區：500 檔股池（手寫 seed + 自動清理補齊到 500；不走外部 HTTPS）
# ✅ 貼在：你原本「pool_500 = {...}」那一段的位置
# 步驟：
#   A) 先把你原本 pool_500 = {...} 改名成 POOL_500_SEED = {...}（內容不改）
#   B) 然後把下面整段貼在 POOL_500_SEED 後面
# ==============================================================================
...
# ---------------------------
FIN_KEYWORDS = [
    "金融保險", "金控", "銀行", "證券", "保險", "壽", "票券", "期貨", "投信", "投顧"
]
...
BUCKET_RULES = [
    ...
    ("🎮 數位內容/軟體/遊戲",      ["文化創意"]),
    ("🧬 生技/醫療",              ["生技醫療"]),
    ("🔋 電力/綠能/儲能",          ["油電燃氣", "電器電纜", "綠能", "環保", "電機機械"]),
    ("🚗 車用/工控/自動化",        ["汽車"]),
    ("⚓ 航運/物流/航空",          ["航運"]),
    ("🏗️ 建材/營建/資產",          ["建材營造"]),
    ("🧱 原物料/化工/鋼鐵",        ["鋼鐵", "塑膠", "化學", "橡膠", "玻璃陶瓷", "造紙"]),
    ("🛒 內需消費/零售/餐旅",      ["食品", "貿易百貨", "觀光餐旅", "紡織", "居家生活", "運動休閒"]),
]
...
assert sum(QUOTAS_500.values()) == 500


def _code_from_ticker(tid: str) -> str:
    return str(tid).strip().upper().split(".")[0]

def _is_code_valid(code: str) -> bool:
    return bool(re.fullmatch(r"\d{4}", str(code)))

def _is_financial(group: str, name: str) -> bool:
    s = f"{group or ''} {name or ''}"
    return any(k in s for k in FIN_KEYWORDS)

def _to_yf_ticker(code: str, market: str) -> str:
    # twstock: market 通常是 "上市"/"上櫃"
    return f"{code}.TWO" if "上櫃" in (market or "") else f"{code}.TW"

def _assign_bucket(group: str) -> str:
    g = group or ""
...
def rebuild_pool_500_from_seed(seed_pool: dict, target_size: int = 500):
    """
    1) 用 twstock.codes 驗證有效代號（不走外部 HTTPS，避開 SSL）
    2) 去金融股
    3) 去重
    4) 依 QUOTAS_500 分桶補齊到 500
    """
    import twstock

    # --- 0) 建 universe（全市場有效非金融）
    universe = []
    for code, info in twstock.codes.items():
        code = str(code)
        if not _is_code_valid(code):
            continue
        ...
        if _is_financial(group, name):
            continue

        bucket = _assign_bucket(group)
        universe.append({
            "code": code,
            "name": name,
            "market": market,
            "group": group,
            "bucket": bucket,
            "market_rank": 0 if "上市" in market else 1,
            "code_int": int(code),
        })

    uni_df = pd.DataFrame(universe).drop_duplicates(subset=["code"]).sort_values(
        ["market_rank", "code_int"], ascending=[True, True]
    )

    # --- 1) 先處理 seed：踢金融/無效/重複
    removed_invalid = []
    removed_fin = []
    kept_seed = []

    seen = set()
    flat_seed = [(k, tid, nm) for k, lst in seed_pool.items() for (tid, nm) in lst]

    for _, tid, _nm in flat_seed:
        code = _code_from_ticker(tid)

        if not _is_code_valid(code) or code not in twstock.codes:
            removed_invalid.append(tid)
            continue

        info = twstock.codes[code]
        name = getattr(info, "name", "") or _nm or ""
        market = getattr(info, "market", "") or ""
        group = getattr(info, "group", "") or ""

        if _is_financial(group, name):
            removed_fin.append(_to_yf_ticker(code, market))
            continue
    ...
    seed_df = pd.DataFrame(kept_seed)
    if seed_df.empty:
        seed_df = pd.DataFrame(columns=["code","tid","name","bucket","market_rank","code_int"])

    # --- 2) 依配額組裝：先塞 seed，再補 universe
    picked = []
    picked_codes = set()

    for bucket, q in QUOTAS_500.items():
        # 先放 seed（保留你原始偏好）
        ssub = seed_df[seed_df["bucket"] == bucket]
        for _, r in ssub.iterrows():
            if r["code"] in picked_codes:
                continue
            picked.append((bucket, r["tid"], r["name"]))
            picked_codes.add(r["code"])
            if sum(1 for x in picked if x[0] == bucket) >= q:
                break

        # 不足就用 universe 補
        need = q - sum(1 for x in picked if x[0] == bucket)
        if need > 0:
            cand = uni_df[(uni_df["bucket"] == bucket) & (~uni_df["code"].isin(picked_codes))].head(need)
            for _, r in cand.iterrows():
                picked.append((bucket, _to_yf_ticker(r["code"], r["market"]), r["name"]))
                picked_codes.add(r["code"])

    # --- 3) 若仍不足 500：從全市場剩餘補到 500（輪流補到熱門桶）
    if len(picked) < target_size:
        rest = uni_df[~uni_df["code"].isin(picked_codes)]
        cycle = [
            "🔬 半導體/IC/設備",
            "🌬️ AI伺服器/電腦週邊",
            "📡 通訊/網通/資安",
        ...
        for _, r in rest.iterrows():
            if len(picked) >= target_size:
                break
            b = cycle[i % len(cycle)]
            picked.append((b, _to_yf_ticker(r["code"], r["market"]), r["name"]))
            picked_codes.add(r["code"])
            i += 1
    ...
    # --- 4) 組回 dict（key 顯示配額）
    out = {}
    for bucket, tid, name in picked:
        out.setdefault(f"{bucket} ({QUOTAS_500.get(bucket, 0)})", []).append((tid, name))

    report = {
        "seed_total": sum(len(v) for v in seed_pool.values()),
        "seed_kept_non_fin_valid_unique": int(len(seed_df)),
        "removed_financial": int(len(set(removed_fin))),
        "removed_invalid": int(len(set(removed_invalid))),
        "final_total": int(sum(len(v) for v in out.values())),
    }
    return out, report


# ---------------------------
# 3) session_state 快取：避免每次 rerun 都重建股池
# ---------------------------
def get_pool_500_safe():
    ...
    try:
        p, rep = rebuild_pool_500_from_seed(POOL_500_SEED, target_size=500)
        st.session_state.pool_report = rep
    except Exception as e:
        st.sidebar.error(f"⚠️ 自動生成股池失敗，改用手寫股池（可能不是500，僅救命用）：{e}")
        p = POOL_500_SEED
...
# （可選）debug：側欄顯示清理報告
# st.sidebar.write(st.session_state.get("pool_report", {}))
