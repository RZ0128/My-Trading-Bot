...
with tab_brain:
    ...
    with st.container(border=True):
        ...
        # 按鈕恢復為掃描股池標籤
        if st.button("📡 啟動偵察機：即時獵殺 500 檔核心標的", width="stretch", key="hunt_v187_fixed"):
            found_heroes = []
            all_targets = []
    ...
    with st.expander("📝 AI 自我診斷與實戰修正", expanded=True):
        st.write("目前狀態：數據獵食中... 🟢 (正在建立您的英雄百科全書)")
