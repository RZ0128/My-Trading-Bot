...
# --- CONFIG ---
st.set_page_config(page_title="AI Manager 6.4", layout="wide")
...
with st.sidebar:
    ...
    cur_c = st.selectbox("🎯 選取帳戶", all_c if all_c else ["請先建立客戶"])

# --- MAIN UI ---
st.title(f"🛡️ AI 經理人 6.4：[{cur_c}] 戰情室")

col_l, col_r = st.columns([1.8, 1.2])

with col_l:
    st.subheader("🔥 每日 15 檔推薦")
    scan_list = [
        {"id": "2402.TW", "name": "毅嘉", "reason": "突破前波大量區，MACD日線轉正。"},
        {"id": "6531.TW", "name": "愛普*", "reason": "月日MACD共振，起漲第一點。"},
        {"id": "3035.TW", "name": "智原", "reason": "慣性改變，紅K收復大量區高點。"},
        {"id": "5269.TW", "name": "祥碩", "reason": "帶量突破年線，溢價預估 20%+"},
        {"id": "3227.TW", "name": "原相", "reason": "60分K回測不破，均線斜率向上。"},
        {"id": "3034.TW", "name": "聯詠", "reason": "低位MACD收斂，高殖利率護體。"},
        {"id": "2603.TW", "name": "長榮", "reason": "運價支撐，月線多頭排列。"},
        {"id": "2317.TW", "name": "鴻海", "reason": "228元防線確立，GB200量產。"},
        {"id": "6438.TW", "name": "迅得", "reason": "CoWoS噴發，MACD零軸上金叉。"},
        {"id": "3661.TW", "name": "世芯-KY", "reason": "殺盤結束，法人重回成本區。"},
        {"id": "2330.TW", "name": "台積電", "reason": "核心資產，月日MACD同步發散。"},
        {"id": "2454.TW", "name": "聯發科", "reason": "邊緣AI題材，站穩所有均線。"},
        {"id": "6271.TW", "name": "同欣電", "reason": "低軌衛星題材，均線斜率向上。"},
        {"id": "3008.TW", "name": "大立光", "reason": "底背離完成，主力低位吃貨。"},
        {"id": "2308.TW", "name": "台達電", "reason": "能源管理看好，季線支撐強。"}
    ]
...
with col_r:
    ...
    if cur_c in st.session_state.client_battles and st.session_state.client_battles[cur_c]:
        ...
        for itm in st.session_state.client_battles[cur_c]:
            ...
            p_list.append({"標的":itm['name'], "損益%":f"{pct:+.2f}%", "指令":adv})
        st.table(pd.DataFrame(p_list))
        st.metric("累積損益 (TWD)", f"{total_p:+.2f}")
        if st.button("清空"): st.session_state.client_battles[cur_c] = []; st.rerun()
    else: st.info("尚無持股")
...
st.divider()
st.subheader("📊 戰略決策圖解")
st.caption("🚨 警示：頂背離（股價創高但動能衰退，建議落袋為安）")
st.caption("🛑 警示：破位（股價跌破關鍵支撐，建議停損）")
